...
#define TOTAL_STACK_SIZE_NP (STACK_SIZE_NP + STACK_GUARD_BAND)
#else
#define STACK_SIZE_NP 0
#define TOTAL_STACK_SIZE_NP 0
...
MEMORY
{
  ...
  RAM_S    (rwx) : ORIGIN = (SRAM_ORIGIN + SECURE_ALIAS_OFFSET + SRAM_OFFSET),
                   LENGTH = UVISOR_SRAM_LENGTH_USED - TOTAL_STACK_SIZE - TOTAL_STACK_SIZE_NP
  STACK_S  (rw ) : ORIGIN = ORIGIN(RAM_S) + LENGTH(RAM_S),
                   LENGTH = UVISOR_SRAM_LENGTH_USED - LENGTH(RAM_S)
}
